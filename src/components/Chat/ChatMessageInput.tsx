...
const ChatMessageInput = ({ disabled, onSend }: Props) => {
    ...
    return (
        <div className={`flex border border-gray-800/50 bg-gray-600/50 p-2 rounded-md ${disabled && 'opacity-50'}`}>
            <textarea
            ...
            <div onClick={handleSendMessage} className={`self-end p-1 cursor-pointer rounded ${text.length ? 'opacity-100 hover:bg-black/200' : 'opacity-20'}`}>
                <IconBxSend width={20} height={20} />
            </div>
        </div>
    );
}
